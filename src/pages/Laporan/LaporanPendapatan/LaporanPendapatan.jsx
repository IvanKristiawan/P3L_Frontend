...
const LaporanPendapatan = () => {
  ...
  const [lapPendapatansData, setLapPendapatansData] = useState([]);
  ...
  const handleGeneratePdf = () => {
    ...
    doc.html(reportTemplateRef.current, {
      async callback(doc) {
        await doc.save("LaporanPendapatan");
      },
  ...
  const tampilPdf = async (e) => {
    ...
    if (form.checkValidity()) {
      let lapPendapatans = await axios.post(`${tempUrl}/laporanPendapatan`, {
        dariTanggal,
        sampaiTanggal,
        _id: user.id,
        token: user.token
  ...
  let keys = Object.keys(lapPendapatansData);

  for (let i = 0; i < lapPendapatansData.length; i++) {
    list.push(
      <tr style={tableText}>
        <td style={textTableRight}>
          {lapPendapatansData[keys[i]].aktivasi.toLocaleString()}
        </td>
        <td style={textTableRight}>
          {lapPendapatansData[keys[i]].deposit.toLocaleString()}
        </td>
        <td style={textTableRight}>
          {lapPendapatansData[keys[i]].total.toLocaleString()}
        </td>
      </tr>
    );
  }
  ...
  return (
    <Container>
      <h3>Laporan</h3>
      <h5 style={{ fontWeight: 400 }}>Laporan Pendapatan</h5>
      <hr />
      <Box sx={spacingTop}>
        ...
        <div style={{ marginTop: "10px" }}>
          ...
          <div ref={reportTemplateRef} id="content" style={pdfContainer}>
            ...
            <table style={{ width: "1000px" }}>
              <tbody>
                <tr>
                  <th style={tableTitle}>Aktivasi</th>
                  <th style={tableTitle}>Deposit</th>
