...
const LaporanGym = () => {
  ...
  const tampilPdf = async (e) => {
    ...
    if (form.checkValidity()) {
      let lapGyms = await axios.post(`${tempUrl}/laporanGym`, {
        dariTanggal,
        sampaiTanggal,
        _id: user.id,
        token: user.token
      });
      setLapGymsData(lapGyms.data);
  ...
  for (let i = 0; i < lapGymsData.length; i++) {
    list.push(
      <tr style={tableText}>
        <td style={{ fontWeight: 700 }}>{lapGymsData[keys[i]].tanggal}</td>
        <td style={textTableRight}>
          {lapGymsData[keys[i]].jumlahMember.toLocaleString()}
        </td>
      </tr>
    );
  }
  ...
  return (
    <Container>
      <h3>Laporan</h3>
      <h5 style={{ fontWeight: 400 }}>Laporan Gym</h5>
      <hr />
      <Box sx={spacingTop}>
        ...
        <div style={{ marginTop: "10px" }}>
          ...
          <div ref={reportTemplateRef} id="content" style={pdfContainer}>
            ...
            <table style={{ width: "1000px" }}>
              <tbody>
                <tr>
                  <th style={tableTitle}>Tanggal</th>
                  <th style={tableTitle}>Jml. Member</th>
                </tr>
                {list}
              </tbody>
            </table>
