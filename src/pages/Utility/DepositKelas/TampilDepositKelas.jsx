...
import { ShowTableDepositKelas } from "../../../components/ShowTable";
...
const TampilDepositKelas = () => {
  ...
  const getDeposits = async () => {
    setLoading(true);
    try {
      const response = await axios.post(`${tempUrl}/depositKelass`, {
        _id: user.id,
        token: user.token,
      });
  ...
  const getDepositById = async () => {
    if (id) {
      const response = await axios.post(`${tempUrl}/depositKelass/${id}`, {
        _id: user.id,
        token: user.token,
      });
  ...
  const deleteDeposit = async (id) => {
    setLoading(true);
    try {
      await axios.post(`${tempUrl}/deleteDepositKelas/${id}`, {
        userId: memberId,
        _id: user.id,
        token: user.token,
      });
  ...
  return (
    <Container>
      <h3>Utility</h3>
      <h5 style={{ fontWeight: 400 }}>Daftar Deposit Kelas</h5>
      <Box sx={buttonModifierContainer}>
        <ButtonModifier
          id={id}
          kode={id}
          addLink={`/depositKelas/tambahDepositKelas`}
          editLink={null}
...
export default TampilDepositKelas;
